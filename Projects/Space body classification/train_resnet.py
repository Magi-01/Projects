...
OUT_DIR    = "/media/Ubunt_2/Project/pipeline_output_resnet"
...
def run_training(model, train_dl, val_dl, epochs, criterion, optimizer,
    ...
    if completed:
        ...
        if last_ep <= freeze_epochs:
            # frozen optimizer matches checkpoint
            optimizer.load_state_dict(
                ckpt["optimizer_state_dict"]
            )

        else:
            print(f"[{label}] checkpoint is in fine-tuning phase")
            print("[optimizer] keeping current optimizer")
